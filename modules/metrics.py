...
def sim_matrix_training(text_embeds, vid_embeds_pooled, pooling_type):
    ...
    text_embeds = text_embeds / text_embeds.norm(dim=-1, keepdim=True)  # [num_texts, 768]
    vid_embeds_pooled = vid_embeds_pooled / vid_embeds_pooled.norm(dim=-1, keepdim=True)  # [num_vids, num_frames, 768]

    # print('test:', text_embeds.shape, vid_embeds_pooled.shape)

    if pooling_type == 'avg':
        # If pooling is average, compute directly using matrix multiplication
        sims = torch.mm(text_embeds, vid_embeds_pooled.t())  # [num_texts, num_vids]
        
    else:
        # For temporal pooling (e.g., sum, max, attention, etc.)
        # Adjust vid_embeds_pooled dimensions for batch matrix multiplication
        vid_embeds_pooled = vid_embeds_pooled.permute(0, 2, 1)  # [num_vids, 768, num_frames]
        text_embeds = text_embeds.unsqueeze(1)  # [num_texts, 1, 768]
        sims = torch.bmm(text_embeds, vid_embeds_pooled).squeeze(1)  # [num_texts, num_vids]
        sims = sims.sum(dim=1, keepdim=True)  # Sum across frames, resulting in [b, 1]
        sims = torch.mm(sims, sims.t())  # Pairwise similarity matrix [b, b]

    return sims
def sim_matrix_it(text_embeds, image_embeds):
    """
    ...
    sims = torch.mm(text_embeds, image_embeds.t())
...
def calculate_recall(similarities, k_values=[1, 5, 10]):
    ...
    """
    recalls = {}
    _, indices = similarities.topk(max(k_values), dim=1, largest=True, sorted=True)
    ground_truth = torch.arange(similarities.size(0)).to(similarities.device)
    for k in k_values:
        correct = (indices[:, :k] == ground_truth.unsqueeze(1)).any(dim=1).float()
        recall_k = correct.mean().item()
